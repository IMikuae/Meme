...
let handler = async (m, { args, usedPrefix, text, command, participants, groupMetadata, conn }) => {
   if (command === 'ikhlasin' || command === 'ikhlas') {
...
else if (command === 'tembak' || command === 'jadian') {
let toM = a => '@' + a.split('@')[0]
...
let ps = groupMetadata.participants.map(v => v.id)
  ...
  try {
    ...
    if(user === conn.user.jid) return conn.reply(m.chat, `_*Maaf Kak, Tapi Saya Sepenuhnya Milik taufik. :')*_`, m)
  ...
  try {
    ...
    if(user === conn.user.jid) return conn.reply(m.chat, `Maaf Kak, Tapi Saya Sepenuhnya Milik taufik`, m)
    let me = m.sender
...
handler.tags = ['fun']
handler.help = handler.command = /^(ikhlasin|ikhlas|cekpacar|tembak|jadian|putus|terima|tolak)$/i
handler.group = true
handler.fail = null
